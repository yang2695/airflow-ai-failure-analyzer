...
SAMPLES = {
    ...
    "Kubernetes Pod Failure": "dag_id=hourly_exports\ntask_id=run_export\ntry_number=2\nERROR - Pod evicted: node was low on ephemeral-storage. CrashLoopBackOff",
    "Disk Full": "dag_id=archive_events\ntask_id=write_partition\ntry_number=1\nERROR - OSError: [Errno 28] No space left on device",
}
...
with st.sidebar:
    ...
    st.metric("Logs checked", len(st.session_state.history))
    st.metric("High or critical", sum(item["severity"] in {"High", "Critical"} for item in st.session_state.history))
    st.caption("The checks cover database, storage, Kubernetes, data quality, APIs, configuration, and Python errors.")
...
if st.button("Check log", type="primary", use_container_width=True):
...
if analysis := st.session_state.get("analysis"):
    ...
    repeat_count = sum(item["failure_fingerprint"] == analysis["failure_fingerprint"] for item in st.session_state.history)
    details = [f"Reference: `{analysis['failure_fingerprint']}`"]
    if analysis["exception_name"]:
        details.append(f"Exception: `{analysis['exception_name']}`")
    if repeat_count > 1:
